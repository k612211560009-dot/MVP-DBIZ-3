...
CREATE TABLE `DONOR` (
  `donor_id` uuid PRIMARY KEY COMMENT '1–1 with USER (shared PK)',
  `home_bank_id` uuid COMMENT 'current affiliation',
...
CREATE TABLE `EHR_DONOR` (
  `donor_id` uuid PRIMARY KEY COMMENT '1–1 with DONOR (shared PK)',
...
CREATE TABLE `VISIT_SCHEDULE` (
  `visit_id` uuid PRIMARY KEY COMMENT '1–1 with DONATION_VISIT (shared PK)',
