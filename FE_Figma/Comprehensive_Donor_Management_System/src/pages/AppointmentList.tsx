...
import { Calendar as CalendarIcon, Filter, CheckCircle, XCircle, Edit } from 'lucide-react';
...
export function AppointmentList() {
  ...
  const getTypeBadge = (type: string) => {
    return (
      <Badge variant="outline">
        {type === 'screening' ? 'Sàng lọc' : 'Hiến sữa'}
      </Badge>
    );
  };
  ...
  const handleMarkAsFailed = () => {
    ...
    // Simulate API call
    setTimeout(() => {
      toast.success(`Đã đánh dấu lịch hẹn ${selectedAppointment.id} thất bại`, {
        description: `Thông báo đã được gửi đến ${selectedAppointment.donorName}`,
      });
        ...
        <div className="flex gap-2">
          ...
          {apt.status === 'arrived' && apt.type === 'donation' && (
            <Button
              variant="default"
              size="sm"
              onClick={() => navigate(`/record-donation?appointmentId=${apt.id}`)}
            >
              <Edit className="h-4 w-4 mr-1" />
              Ghi nhận
            </Button>
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Quản lý lịch hẹn sàng lọc và hiến sữa</p>
        </div>
      ...
      <Dialog open={isFailModalOpen} onOpenChange={setIsFailModalOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Đánh dấu lịch hẹn thất bại</DialogTitle>
            <DialogDescription>
              Bạn có chắc chắn muốn đánh dấu lịch hẹn này là thất bại?
              {selectedAppointment && (
      ...
      <div className="p-4 bg-muted/50 rounded-lg border border-dashed">
        ...
        <ul className="list-disc list-inside space-y-1 text-muted-foreground mt-2">
          <li>POST /api/admin/appointments/{'{id}'}/mark_failed with reason and notes</li>
          <li>Send SMS notification to donor when appointment is marked as failed</li>
          <li>Log all status changes with timestamp and user who made the change</li>
          <li>Calendar mini view shows appointments by date (integrate with date filter)</li>
          <li>Medical Staff can only see appointments where staff = current_user_id</li>
