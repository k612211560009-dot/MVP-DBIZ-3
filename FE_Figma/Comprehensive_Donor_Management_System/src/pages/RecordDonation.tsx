...
export function RecordDonation() {
  ...
  const estimatedPoints = healthStatus === 'good' && volume
    ? calculatePoints(Number(volume))
    : 0;

  const handleSubmit = async () => {
    // Validation
    if (!healthStatus) {
      toast.error('Vui lòng chọn tình trạng sức khỏe');
      return;
    }

    if (healthStatus === 'good') {
      if (!volume || Number(volume) <= 0) {
        toast.error('Vui lòng nhập lượng sữa');
        return;
      }
      if (!containers || Number(containers) <= 0) {
        toast.error('Vui lòng nhập số container');
        return;
      }
    }
    ...
    setTimeout(() => {
      ...
      const points = calculatePoints(actualVolume);
      
      toast.success('Đã ghi nhận lần hiến sữa', {
        description: `${actualVolume}ml đã được ghi nhận. Tặng ${points} điểm cho donor.`,
      });
    ...
    <div className="p-6 space-y-6 max-w-3xl">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <RadioGroup value={healthStatus} onValueChange={(value: any) => {
              ...
              <div className="flex items-center space-x-2">
                ...
                <Label htmlFor="health-good" className="cursor-pointer flex items-center gap-2">
              ...
              <div className="flex items-center space-x-2">
                ...
                <Label htmlFor="health-bad" className="cursor-pointer flex items-center gap-2">
          ...
          {healthStatus === 'bad' && (
            <Alert variant="destructive">
              ...
              <AlertDescription>
                Donor không đủ điều kiện sức khỏe. Lượng sữa và số container sẽ được đặt về 0.
                Vui lòng ghi chú lý do cụ thể.
              </AlertDescription>
            </Alert>
          )}
        </CardContent>
        ...
        <CardContent className="space-y-4">
          ...
          {healthStatus === 'good' && volume && Number(volume) > 0 && (
            <Alert>
              ...
              <AlertDescription>
                Donor sẽ nhận được <strong>{estimatedPoints} điểm</strong> cho lần hiến sữa này.
              </AlertDescription>
            </Alert>
          )}
          ...
          <div className="p-4 bg-muted rounded-lg">
            ...
            <div className="mt-2 space-y-1 text-muted-foreground">
              <p>Donor: {donorInfo.name} ({donorInfo.id})</p>
              <p>Sức khỏe: {healthStatus === 'good' ? '✓ Tốt' : healthStatus === 'bad' ? '✗ Không tốt' : '- Chưa chọn'}</p>
      ...
      <div className="p-4 bg-muted/50 rounded-lg border border-dashed">
        ...
        <ul className="list-disc list-inside space-y-1 text-muted-foreground mt-2">
          <li>POST /api/admin/appointments/{'{id}'}/donation - Save donation record</li>
          <li>Automatically calculate points based on reward rules from system config</li>
          <li>Update donor's total donation volume and point balance</li>
          <li>Send SMS notification to donor with donation summary and points</li>
          <li>If health status = bad, require detailed notes explaining reason</li>
          <li>Container validation: warn if container count doesn't match volume ratio</li>
          <li>Permission: Medical Staff can record, Director can edit/delete records</li>
          <li>Barcode scanner integration for container tracking (future enhancement)</li>
        </ul>
      </div>
    </div>
  );
