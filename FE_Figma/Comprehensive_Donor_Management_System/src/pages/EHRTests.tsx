...
export function EHRTests() {
  ...
  const filteredTests = mockEHRTests.filter(test => {
    const matchesValidity = validityFilter === 'all' || test.validity === validityFilter;
  ...
  const getValidityBadge = (validity: string) => {
    const variants: Record<string, any> = {
      ...
      expiring_soon: { variant: 'secondary', label: 'Sắp hết hạn', icon: AlertTriangle },
      expired: { variant: 'destructive', label: 'Hết hạn', icon: AlertTriangle },
    };
    const config = variants[validity] || { variant: 'secondary', label: validity, icon: AlertTriangle };
  ...
  const expiringCount = filteredTests.filter(t => t.validity === 'expiring_soon').length;
  const expiredCount = filteredTests.filter(t => t.validity === 'expired').length;
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Trích xuất và xác minh kết quả xét nghiệm</p>
        </div>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-4 gap-4">
          ...
          <div>
            ...
            <Badge variant="default">{jobStatus.status === 'completed' ? 'Hoàn thành' : 'Đang chạy'}</Badge>
          </div>
          <div>
            <label className="text-muted-foreground">Số bản ghi trích xuất</label>
        ...
        <div className="flex gap-2">
          {['HIV', 'Hepatitis B', 'Hepatitis C', 'Syphilis'].map(type => (
            <Button
              ...
              size="sm"
              onClick={() => setTestTypeFilter(testTypeFilter === type ? 'all' : type)}
            >
              {type}
            </Button>
          ))}
      ...
      <div className="p-4 bg-muted/50 rounded-lg border border-dashed">
        ...
        <ul className="list-disc list-inside space-y-1 text-muted-foreground mt-2">
          <li>GET /api/admin/ehr-tests/status - Job status and next run time</li>
          <li>POST /api/admin/ehr-tests/extract - Manual trigger extraction job</li>
          ...
          <li>Export list of donors with expired/expiring tests for scheduling</li>
        </ul>
      </div>
    </div>
  );
