...
export function Payments() {
  ...
  const handleMarkTransferred = () => {
    ...
    toast.success(`Đã đánh dấu đã chuyển khoản cho ${selectedPayment.donorName}`, {
  ...
  const getStatusBadge = (status: string) => {
    const variants: Record<string, any> = {
      ...
      transferred: { variant: 'default', label: 'Đã chuyển', icon: CheckCircle },
  ...
  const transferredCount = mockPayments.filter(p => p.status === 'transferred').length;
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground">Quản lý thanh toán cho các donor</p>
      </div>
      ...
      <Dialog open={isTransferModalOpen} onOpenChange={setIsTransferModalOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleMarkTransferred}>
              Xác nhận
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <div className="p-4 bg-muted/50 rounded-lg border border-dashed">
        ...
        <ul className="list-disc list-inside space-y-1 text-muted-foreground mt-2">
          <li>GET /api/admin/payments?status=&page=&per_page= - List payments</li>
          <li>POST /api/admin/payments/{'{id}'}/mark-transferred - Mark as transferred with date and receipt</li>
          <li>File upload: Store receipt in secure file storage (S3, Azure Blob, etc.)</li>
          <li>Send email/SMS notification to donor when payment is marked as transferred</li>
          <li>Payment history: Show all transfers for a specific donor</li>
          <li>Bulk operations: Mark multiple payments as transferred at once</li>
          <li>Integration: Auto-generate payment requests based on accumulated points</li>
          <li>Permission: Only Director/Admin/Finance staff can mark payments as transferred</li>
